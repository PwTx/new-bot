...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Salve Membros Novos..👋 @${num.split('@')[0]}\nBem vindo ao grupo *${mdata.subject}*`
				...
				teks = `Falou Meu Cria, Até Mais... @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Processando, Seja Paciente ⌛',
				success: '✔️ Sucesso ✔️',
                                levelon: '❬ ✔ ❭ *É enorme* nível ativado*',
				leveloff: ' ❬ X ❭  *Eu sinto Muito* Nível Desativado*',
				levelnoton: '❬ X ❭ *Eu sinto Muito* Nível Inativo*',
				levelnol: '*Eu sinto Muito* Seu nível ainda* 0 °-°',
				error: {
					stick: '[❗] *Eu sinto Muito*, ocorreu um erro ao converter a imagem para um sticker ❌',
					Iv: '❌ Link Inválido ❌'
				},
				only: {
					group: '[❗] *Eu sinto Muito..este é um recurso pré-pago* Por favor entre em contato >553398091185 wa.me/553398091185 Untuk pembayaran! ❌',
					ownerG: '[❗] Recurso especiais para dono do grupo ❌',
					ownerB: '[❗] *VOCÊ E LOUCO*? você não é dono do bot! ❌',
					admin: '[❗] Recursos especiais de administrador de grupo..! ❌',
					Badmin: '[❗] *Tornar o bot um administrador  ❌',
		 ...
		 if (!isGroup) return reply(mess.only.group)
		   ...
		   if (!isGroup) return reply(mess.only.group)
				...
				case 'ytmp4':
					...
					if (anu.error) return reply(anu.error)
					teks = `*❏ Titulo* : ${anu.title}\n\n*O vídeo está sendo enviado ... Não Flode..!*`
					thumb = await getBuffer(anu.thumb)
					client.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
